...
export async function awalCommand(args = []) {
  try {
    const { stdout, stderr } = await execFileAsync('npx', ['awal@latest', ...args, '--json'], {
      timeout: 30000,
    ...
    try {
      ...
      return { output: stdout.trim(), stderr: stderr.trim() || undefined };
    }
  } catch (err) {
    if (err.stdout) {
